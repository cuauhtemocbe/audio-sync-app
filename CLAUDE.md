...
- `docker-compose.prod.yml` sirve el build de producción vía `Dockerfile` en el puerto `8080`.
- **`Dockerfile` (producción) es un único proceso Node/Express (`server/index.js`), no nginx — corregido el 2026-09-14.** `server/index.js` se agregó como proxy de ElevenLabs (commit `b3316f3`) pero solo se integró a `docker-compose.dev.yml`; el `Dockerfile` de producción seguía siendo nginx sirviendo `dist/` sin ninguna location para `/api/tts` (nginx respondía 405 a ese POST, no 404 — no hay `location` para ese path). Railway solo tiene un servicio (`audio-sync-app`), así que agregar `ELEVENLABS_API_KEY` a Railway no arreglaba nada: no había ningún proceso ahí que la leyera. Fix: la imagen final del `Dockerfile` ahora es `node:26-alpine` corriendo `node server/index.js` (que ya servía `dist/` vía `express.static` Y `/api/tts` desde el mismo proceso, pensado para esto desde que se escribió) — sin nginx, sin segundo servicio en Railway. Etapa `deps` separada instala solo dependencies de producción (`pnpm install --frozen-lockfile --prod`) para no cargar devDependencies (vite, vitest, eslint, ...) en la imagen final.
